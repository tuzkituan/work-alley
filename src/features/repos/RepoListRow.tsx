...
export function RepoListHeader() {
  return (
    ...
    >
      ...
      <span className="wa-col-optional">blazeup-ui</span>
      <span>Dev</span>
      ...
      {status ? (
        <span
          className={cn(
            ...
            uiMismatch ? 'text-sev-warn' : 'text-adaptive-500'
          )}
          title={`blazeup-ui ${status.uiDep.resolved ?? 'n/a'}`}
        >
          {status.uiDep.resolved ?? (status.uiDep.declared ? 'n/a' : '—')}
        </span>
      ) : (
        <Skeleton className="wa-col-optional h-3 w-12" />
      )}
